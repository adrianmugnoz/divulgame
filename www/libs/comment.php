...
class Comment extends LCPBase {
	...
	function print_summary($link=0, $length=0, $single_link=true) {
		...
		if ($globals['now'] - $this->date > 86400) {
			$this->author_info = sprintf(_('%s %s el %s'), $edited, $author, get_date_time($this->date));
		} else {
			$this->author_info = sprintf(_('%s %s hace %s'), $edited, $author, txt_time_diff($this->date));
		}
...
//TODO
	...
	static function print_form($link, $rows=12) {
		...
		if($link->date < $globals['now']-$globals['time_enabled_comments'] || $link->comments >= $globals['max_comments']) {
			...
			echo '<label for="comment">'. _('Añade un comentario').'<br /></label>'."\n";
